...
class AggregationPlugin(BaseAdminPlugin):
    ...
    cust_aggregate_fields = {}

    def init_request(self, *args, **kwargs):
        return bool(self.cust_aggregate_fields)

    def _get_field_aggregate(self, field_name, obj, row):
        item = ResultItem(field_name, row)
        item.classes = ['aggregate', ]
        if field_name not in self.cust_aggregate_fields:
            item.text = ""
        else:
            try:
                f = self.opts.get_field(field_name)
                agg_method = self.cust_aggregate_fields[field_name]
                ...
                if key not in obj:
                    ...
                    trans_info = '%%s<span class="aggregate_title label label-info">%s</span>'
                    if agg_method == 'min':
                        trans_info_format = trans_info % _("Min")
                    elif agg_method == 'max':
                        trans_info_format = trans_info % _("Max")
                    elif agg_method == 'avg':
                        trans_info_format = trans_info % _("Avg")
                    elif agg_method == 'sum':
                        trans_info_format = trans_info % _("Sum")
                    elif agg_method == 'count':
                        trans_info_format = trans_info % _("Count")
                    else:
                        trans_info_format = trans_info % ''
                    item.wraps.append(trans_info_format)
    ...
    def _get_aggregate_row(self):
        ...
        obj = queryset.aggregate(*[AGGREGATE_METHODS[method](field_name) for field_name, method in
                                   self.cust_aggregate_fields.items() if method in AGGREGATE_METHODS])
